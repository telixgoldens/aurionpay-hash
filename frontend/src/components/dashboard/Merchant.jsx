...
import { createHSPOrderDirect } from "../../utils/hspClient.js";
...
export default function Merchant({ address, signer, addLog }) {
  ...
  // HSP: Create order directly from browser (bypasses Cloudflare on Render)
  const createHSPOrder = useCallback(async () => {
    ...
    const orderId  = `AURION-${Date.now()}-${rand}`;
    const payReqId = `PAY-${Date.now()}-${rand}`;
    let amt;
    try { amt = ethers.parseUnits(amount, tokenInfo.decimals); }
    catch { return setStatus({ text: "Invalid amount", type: "error" }); }
    ...
    try {
      ...
      const order = await createHSPOrderDirect({
        orderId,
        ...
        <div className="card">
          ...
          {mode && !invoiceId && !hspUrl && (
            <>
              ...
              {/* Invoice Note -- shown on both ZK and HSP */}
              <div>
              ...
              <div className={`status-bar ${paid ? "success" : "pending"}`} style={{ marginBottom: "10px" }}>
                ...
                <span>
                  {paid
                    ...
                    : polling
                    ? (mode === "hsp" ? "Polling HSP for payment..." : t("watchingPayment", lang))
                    : status.text}
                </span>
              </div>
              {invoiceNote && (
